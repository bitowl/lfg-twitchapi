...
module.exports = function(extensionApi) {
  ...
  if (!nodecg.config.login.enabled) {
    throw new Error(
      "Login security is not enabled, lfg-twitchapi will not load."
    );
  }

  if (!nodecg.config.login.twitch.enabled) {
    throw new Error(
      "Twitch authentication is not enabled, lfg-twitchapi will not load."
    );
  }
  ...
  const sessionReplicant = nodecg.Replicant("session", {
    defaultValue: null,
    persistent: false
  });
  ...
  loginLib.on("login", session => {
    ...
    if (
      ...
      user.username === nodecg.bundleConfig.username
    ) {
  ...
  if (requestOptions.url.indexOf("{{username}}") >= 0) {
    ...
    if (!accessToken) {
      return new Promise((resolve, reject) => {
        process.nextTick(() => {
          reject(
            `Access token for "${nodecg.bundleConfig.username}" has not been captured yet.` +
